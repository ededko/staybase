...
export default async function InternalRequestsPage() {
  ...
  const [requests, hostels, members] = await Promise.all([
    prisma.internalRequest.findMany({
      where: { workspaceId: workspace.id, ...(isStaff ? { requestedByUserId: session.user.id } : {}) },
  ...
  return <main className="operations-page p-4 sm:p-6 lg:p-8">
    ...
    {!isStaff && <div className="mini-metrics mt-5"><div><b>{requests.filter((item) => item.status === "NEW").length}</b><span>Нових</span></div><div><b>{requests.filter((item) => item.status === "REVIEW").length}</b><span>На розгляді</span></div><div><b>{requests.filter((item) => item.status === "IN_PROGRESS").length}</b><span>У роботі</span></div></div>}
    <section className="operations-form mt-6 rounded-2xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold">+ Подати заявку</h2>
