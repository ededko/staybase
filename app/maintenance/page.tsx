...
export default async function MaintenancePage() {
  ...
  const canViewAll = permissions.includes("MAINTENANCE_VIEW");
  const canViewAssigned = permissions.includes("MAINTENANCE_ASSIGNED");
  const canCreate = permissions.includes("MAINTENANCE_CREATE");
  const canEdit = permissions.includes("MAINTENANCE_EDIT");
  if (!canViewAll && !canViewAssigned) throw new Error("У вас немає дозволу на перегляд ремонтів");
  const [tickets, hostels, members] = await Promise.all([
    prisma.maintenanceTicket.findMany({ where:{workspaceId:workspace.id,...(!canViewAll?{assignedToUserId:session.user.id}:{})}, include:{hostel:true,room:true}, orderBy:[{status:"asc"},{createdAt:"desc"}] }),
    prisma.hostel.findMany({ where:{workspaceId:workspace.id}, include:{rooms:{orderBy:{name:"asc"}}}, orderBy:{name:"asc"} }),
    prisma.workspaceMember.findMany({where:{workspaceId:workspace.id,role:{not:"OWNER"}},include:{user:{select:{id:true,name:true,email:true}}},orderBy:{createdAt:"asc"}}),
  ]);
  ...
  return <main className="operations-page p-4 sm:p-6 lg:p-8">
    <header className="operations-header"><div><p className="eyebrow">ОБСЛУГОВУВАННЯ</p><h1 className="text-4xl font-bold">Ремонтні заявки</h1><p className="mt-2 text-slate-500">Поломки, постачання та завдання для команди</p></div>{canCreate&&<Link className="primary-action rounded-xl px-5 py-3 font-semibold" href="/maintenance/qr">QR-коди</Link>}</header>
    ...
    {canCreate&&<section className="operations-form mt-6 rounded-2xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold">+ Створити заявку вручну</h2>
      <form action={createMaintenanceTicket} className="form-grid mt-4">
        ...
        <label>Призначити працівника<select name="assignedToUserId"><option value="">Призначити пізніше</option>{members.map(m=><option key={m.user.id} value={m.user.id}>{m.user.name} · {m.user.email}</option>)}</select></label><label>Додати фото<input name="photo" type="file" accept="image/*" capture="environment" /></label>
