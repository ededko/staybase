  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-2">
        <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-2xl font-bold">Особисті дані</h2><dl className="mt-5 space-y-3 text-slate-700"><div><dt className="text-sm text-slate-500">Телефон</dt><dd>{resident.phone || "—"}</dd></div><div><dt className="text-sm text-slate-500">Email</dt><dd>{resident.email || "—"}</dd></div><div><dt className="text-sm text-slate-500">Заселення</dt><dd>{resident.checkIn.toLocaleDateString("uk-UA")}</dd></div><div><dt className="text-sm text-slate-500">Плановий виїзд</dt><dd>{resident.checkOut?.toLocaleDateString("uk-UA") || "—"}</dd></div></dl></section>
